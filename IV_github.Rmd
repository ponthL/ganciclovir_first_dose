...
auc_ml_split <- initial_split(auc_ml_IV,strata = events)
auc_ml_train <- training(auc_ml_split)
auc_ml_test  <- testing(auc_ml_split)


summary(auc_ml_train)
...
##hyperparameters
set.seed(2345)
folds <- vfold_cv(auc_ml_train, strata = events)#par défaut 10 fois
```
...
dose_search <- function(Pds, taille, Clcreat, SEX, greffe, age) {
  amt2 <- ifelse(Clcreat < 150, 5*Pds, 5*Pds)
...
res_dosesearch_test_IV <- research_test_bis %>% mutate(proba=predict(auc_ml_model_st, research_test_bis,type="prob")[[2]])
